package com.across.senzec.datingapp.instagram;
...
import com.across.senzec.datingapp.lazyload.Utils;
...
//		mProgress = new ProgressDialog(context);
		mProgress = new ProgressDialog(context, R.style.MyTheme);
...
//		mProgress.setMessage("Getting access token ...");
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					prefs.putString(App.Key.INSTA_ACCESS_TOKEN,mAccessToken);
	...
	public void fetchUserName(final Handler handler) {
		mProgress = new ProgressDialog(mCtx,R.style.MyTheme);
		mProgress.setCancelable(false);
		mProgress.setProgressStyle(android.R.style.Widget_ProgressBar_Small);
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					userInfo.put(Constants.TAG_USERID, data_obj.getString(Constants.TAG_USERID));

					userInfo.put(Constants.TAG_PROFILE_PICTURE,
							data_obj.getString(Constants.TAG_PROFILE_PICTURE));
					...
					userInfo.put(Constants.TAG_FOLLOWED_BY,
							counts_obj.getString(Constants.TAG_FOLLOWED_BY));

					userInfo.put(Constants.TAG_MEDIA, counts_obj.getString(Constants.TAG_MEDIA));

					userInfo.put(Constants.TAG_FULL_NAME,
							data_obj.getString(Constants.TAG_FULL_NAME));
			...
			//*************Store data into SharedPreference and Constant class*****************


					prefs.putString(App.Key.ID_LOGGED, data_obj.getString(Constants.TAG_USERID));
					prefs.putString(Constants.TAG_USERNAME, data_obj.getString(Constants.TAG_USERNAME));
					prefs.putString(Constants.TAG_PROFILE_PICTURE,data_obj.getString(Constants.TAG_PROFILE_PICTURE));
					prefs.putString(Constants.TAG_BIO, data_obj.getString(Constants.TAG_BIO));
					prefs.putString(Constants.TAG_WEBSITE, data_obj.getString(Constants.TAG_WEBSITE));
					prefs.putString(Constants.TAG_FULL_NAME,data_obj.getString(Constants.TAG_FULL_NAME));
...
//		 mCtx.startActivity(webAuthIntent);
	...
	public void getTempInstaUserId(final Handler handler){
		new Thread(new Runnable() {
			...
			public void run() {
				try{
					...
					JSONObject data_obj = jsonObj.getJSONObject(Constants.TAG_DATA);
					prefs.putString(App.Key.TEMP_INSTA_ID_LOGGED, data_obj.getString(Constants.TAG_USERID));

					handler.sendEmptyMessage(InstagramApp.WHAT_FINALIZE);
